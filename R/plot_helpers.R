...
# Checks or sets lim.x and lim.y for mbo Plots. lim.x and lim.y are named lists,
...
# If NULL, the corresponding limits are calculated via the range of the corresponding
# values.

# X limits for diagnostic plots will NOT be set. Since we have the number of
# iterations on the X axis here, the ggplot default always does what we want.
...
getLimits <- function(lim.x, lim.y, result, iters, extra.measure, ref.point) {
  ...
  # lim.x is the iteration number here. If NULL, use ggplot defaults, else check.
