...
#' What is displayed is documented here: \code{\link{autoplot.ParEGOExampleRun}}.
#'
...
#' @return [\code{ParEGOExampleRun}]
exampleRunParEGO = function(fun, par.set, learner, control, points.per.dim = 50,
  ...
  if (ny != 2L)
    stopf("exampleRunParEGO can only be applied for functions with 2 objectives, but you have %iD", ny)
  if (n.params != 2L)
    stopf("exampleRunParEGO can only be applied for functions with 2 parameters, but you have %iD", ny)
  ...
  parego.hypervolume = dominated_hypervolume(t(res$pareto.front), ref.point)
  ...
  makeS3Obj(c("ParEGOExampleRun", "MBOExampleRun"),
    ...
    names.y = names.y,
    learner = learner,
    parego.hypervolume = parego.hypervolume,
    control = control,
...
print.ParEGOExampleRun = function(x, ...) {
  ...
  catf("Number of objectives        : %i", x$n.params)
  catf("Objective names             : %s", collapse(x$names.y))
  print(x$control)
  catf("Learner                     : %s", x$learner$id)
  catf("Learner settings:\n%s", mlr:::getHyperParsString(x$learner))
  catf("ParEGO Hypervolume          : %.4e", x$parego.hypervolume)
