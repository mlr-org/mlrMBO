...
autoplotExampleRun1d = function(x, iters, xlim, ylim, pause, point.size, line.size, densregion=TRUE...) {
  # FIXME: add comments
  ...
  for (i in iters) {
    ...
    if (par.types %in% c("numeric", "numericvector")) {
      ...
      pl.points = pl.points + ggtitle(sprintf("Iter = %i, Gap = %.4e", i,
                                              calculateGap(opt.path[pastpresent,], global.opt, control)))
