...
#' @param ... [any]\cr
#'   Currently not used.
#' @return [\code{list}]. List of lists. For each iteration the sublist contains seperate ggplot
...
autoplot.ParEGOExampleRun = function(run, iters, pause = TRUE, y1lim = NULL, y2lim = NULL, ...) {
  ...
  mbo.res = run$mbo.res
  nsga2.paretofront = run$nsga2.paretofront
  nsga2.paretoset = run$nsga2.paretoset
  ...
  for (i in iters) {
    for (j in 1:proppoints) {
      ...
      idx = c(idx.init, idx.seq, idx.proposed, idx.nsga2.paretofront)
