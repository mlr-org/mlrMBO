...
proposePointsDIB = function(opt.state) {
  ...
  if (control$propose.points == 1L) {
    ...
    z = createRandomCBControls(control, makeMBOInfillCritDIB(), user.lambda = TRUE)
    props = list()
    ...
    for (i in seq_len(control$propose.points)) {
      ...
      cb = evalCritFunForMultiCritModels(makeMBOInfillCritCB(), prop$prop.points, models, control2,
