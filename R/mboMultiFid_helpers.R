generateMBOMultiFidDesign = function(par.set, control){
  ...
  k = length(par.set$pars[[control$multiFid.control$perf.param]]$values)
  b = ceiling(budget / k)
  ...
  designs = lapply(seq_along(ns), function(i) {
    ...
    des[[control$multiFid.control$perf.param]] = par.set$pars[[control$multiFid.control$perf.param]]$values[[i]]
    des
  })
  res = do.call(rbind.data.frame, designs)
  res[[control$multiFid.control$perf.param]] = factor(res[[control$multiFid.control$perf.param]]) #FIXME: Not the BEST
  res
}
...
#'   How many evaluations should be done on the last value of perf.param.
#' @return \code{MBOMultiFidControl}
#' @export
makeMBOMultiFidControl = function(perf.param, cor.grid.points = 10L, costs = NULL, force.last.level.evals = 10L){
  checkArg(perf.param, "character", len = 1L)
...
infillCritMultiFid2 = function(points, model, control, par.set, design, model.cor, model.sd, model.cost, ...) {
  ...
  alpha1 = replaceByList(points[[control$multiFid.control$perf.param]], model.cor)
  ...
  model.sd.vec = replaceByList(points[[control$multiFid.control$perf.param]], model.sd)
  alpha2 = 1 - (sqrt(2) * model.sd.vec / sqrt(se^2 + model.sd.vec^2))
  alpha3 = replaceByList(points[[control$multiFid.control$perf.param]], model.cost)
