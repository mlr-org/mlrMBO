  ...
  if (!is.null(xs.schedule.info$times)) {
    ...
    if (length(x.times) >= 2){
      ...
      if(typeof(x.prios) != "integer"){
                x.prios = rank(x.prios)
      }
      ks.prio = x.prios
      ks.time = as.integer(x.times * 100)
      ks.cap = as.integer(max.time * 100)
      if (requireNamespace("adagio")){
        ks.vector = adagio::mknapsack(ks.prio, ks.time, ks.cap, bck = 1000)
      }else{
