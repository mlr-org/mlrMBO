...
updateSMBO = function(opt.state, x, y) {
  ...
  infill.values = control$infill.crit$fun(points = x, models = getOptStateModels(opt.state)[[1]], control = control, design = convertOptPathToDf(opt.state), attributes = TRUE, iter = getOptStateLoop(opt.state))
