...
makeScalarTask = function(design, par.set, y.name, control) {
  ...
  design = cbind(design, setColNames(data.frame(y.scalarized), "y.scalarized"))
  design = design[, -which(names(design) %in% control$y.name)]
  makeRegrTask(target="y.scalarized", data=design)
}
