...
# FIXME: add a test with discrete and factor vecs
...
test_that("mbo works with factor variables", {
  ...
  ctrl = makeMBOControl(init.design.points=20, iters=2, infill.opt.random.points=100)
  or = mbo(f, ps, learner=learner, control=ctrl, show.info=FALSE)
  expect_true(!is.na(or$y))
  expect_equal(or$y, f(or$x))
  expect_equal(getOptPathLength(or$opt.path), 20+2)
})
