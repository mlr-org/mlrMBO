...
library(data.table)
reg = loadRegistry("~/lido_nobackup/old_rambo")
...
res.dt[, ':='(cumsum.exec.time = cumsum(exec.time), cummin.y = cummin(y), exec.timestamp.rel = exec.timestamp - min(exec.timestamp)), by = .(prob, algo, repl, probpar.fold, algopar.config.name)]
res.dt[, ':='(dob.exec.timestamp.rel = min(exec.timestamp.rel)), by = .(prob, algo, repl, probpar.fold, algopar.config.name, dob)]
```
...
Zeitverlauf
```{r}
g = ggplot(res.dt[dob >= 1 & algopar.config.name %in% c("r.lcb", "q.lcb")], aes(y = cummin.y, x = cumsum.exec.time, color = algopar.config.name))
...
```{r}
#how it was planned
g = ggplot(res.dt[dob > 0], aes(y = scheduled.on, x = dob.exec.timestamp.rel + scheduled.at, color = selected.learner))
