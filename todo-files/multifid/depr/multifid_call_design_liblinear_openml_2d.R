...
lrn1.par.set = makeParamSet(
  ...
  makeNumericParam("epsilon", lower = 0, upper = 1, trafo = function(x) x^2)
)
...
# [1] 0.02177915
result$proposed
#        cost   epsilon dw.perc
# 1 -2.361238 0.9366261       1
result$y
# [1] 0.02209392

# TUNE DISCRETE ####
ps.disc = makeParamSet(makeDiscreteParam("cost", values = 2^seq(-10,10,length.out=10)),
                       makeDiscreteParam("epsilon", values = seq(0.01,1,length.out=10)^2),
                       makeDiscreteParam("dw.perc", values = 1))
ctrl = makeTuneControlGrid()
r = tuneParams(lrn2, task = task, resampling = rinst, 
               par.set = ps.disc, control = ctrl)
# [Tune] Result: cost=2.16011947778461; epsilon=0.1156; dw.perc=1 : mmce.test.mean=0.0206
log2(r$x$cost)
#1.111111
sqrt(r$x$epsilon)
#0.34
...
save.image("multifid_steps_liblinear_openml_2d.RData")
