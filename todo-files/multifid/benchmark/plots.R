genPlotCompareMbos = function(opt.path.grids, opt.path.cheap, opt.path.expensive) {
	...
	df.grid = do.call(rbind.data.frame, ops.res$op.dfs)
	df.mbo = rbind.data.frame(
		cbind.data.frame(df.cheap, .multifid.lvl = 1), 
		cbind.data.frame(df.expensive, .multifid.lvl = length(opt.path.grids))
		)
	g = ggplot(
		df.mbo, 
