...
openML.res = lapply(openML.ids, openMLBenchmark, e.seed = e.seed, e.lrn = e.lrn, e.par.set = e.par.set, e.lvl = e.lvl, alpha2fix = FALSE, grid.all = FALSE)
...
sasens.res = lapply(names(sasenas), function(sn) {
  ...
  generalBenchmark(e.name = sn, objfun = objfun, e.seed = e.seed, e.par.set = e.par.set, e.lvl = e.lvl, alpha2fix = FALSE, grid.all = FALSE)
})
