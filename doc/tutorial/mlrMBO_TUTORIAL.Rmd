...
This will be discussed und illustrated in section [Experiments and Output](http://berndbischl.github.io/mlrMBO/..)
...
* ``aei``: augmented expected improvement, as designed by [Zitat_Huang](link), is especially useful for the noisy functions,
...
size of this design by ``control$infill.opt.focus.points`` attribute of ``MBOControl`` object (default value is 10000). In the second step the parameter
...
iteratively ``control$infill.opt.focus.maxit`` times (default is 5).
...
The population size, mu, can be set by ``iteration control$infill.opt.ea.mu`` attribute (default value is 10). 
(mu+1) means that in each population only one child is generated using crossover und mutation operators (from **emao** package).  
Parameters ``eta`` and ``p`` of these both operators can be adjust via ``control$infill.opt.ea.sbx.eta``, ``control$infill.opt.ea.sbx.p``,
``control$infill.opt.ea.pm.eta`` and ``control$infill.opt.ea.pm.p`` attributes respectevely. 
The default number of EA iterations is 500 and can be changed by ``infill.opt.ea.maxit`` attribute.

As all three search optimization strategies do not guarantee  to find the global optimum, users can set the number of restarts by 
``control$infill.opt.restarts`` attribute (default value is 1). 
...
Please note that just ``focus.search`` optimizer operates if there are some factor parameters in the parameter set!
...
### TODO 
1) Probleme mit init.design.args 

2) #FIXME: if multipoint.method="cl", check that surrogate model is kriging

3) noisy optimization example
