...
We demonstrate in this tutorial optimization of two simple functions: 5 dimensional ``ackley function`` (``objfun1``) and a self-constructed sine und cosine combination (``objfun2``). 
...
We aim to maximize ``objfun2``. In a following section will be shown how to set the ``MBOControl`` object in order to switch the minimization problem into a maximization one.
...
Furthermore, the user can specify whether the function has to be minimized or maximized with the attribute ``minimize`` (default setting TRUE , e.g.,  minimization).
<!-- FIXME minimize/maximize not found yet.  -->
