...
test_that("plot single-objective", {
  ...
  ctrl = setMBOControlInfill(ctrl, crit = makeMBOInfillCriterionEI(), opt.focussearch.points = 100L,
...
test_that("plot multi-objective", {
  ...
  ctrl = setMBOControlTermination(ctrl, iters = 2L)
  ctrl = setMBOControlInfill(ctrl, crit = getMBOInfillCriterionEI(), opt.focussearch.points = 100L,
