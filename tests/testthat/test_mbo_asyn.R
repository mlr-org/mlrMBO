...
test_that("asyn MBO works with CL", {
  ...
  save.file = file.path(tempdir(), "mbo_asyn", "mbo.RData")
  ctrl = makeMBOControl(schedule.method = "asyn", save.file.path = save.file, propose.points = 1, schedule.nodes = 2, asyn.wait.for.proposals = FALSE, asyn.filter.proposals = TRUE)
