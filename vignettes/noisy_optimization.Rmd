...
Note that Kriging requires heteroscedasticity but slight dependencies of the variance on the `x` can be acceptable like in the following example:
...
The final acquistion function is then similar to the _Expected Improvement_:
\(
\newcommand{\xv}{\boldsymbol{x}}       % observed x vector
\newcommand{\yv}{\boldsymbol{y}}       % observed y vector
\newcommand{\fh}{\hat{f}}              % surrogate model
\newcommand{\fx}{f(\xv)}               % bbox function
\newcommand{\sh}{\hat{s}}              % local estimated uncertainty
\newcommand{\shx}{\sh(\xv)}            % local estimated uncertainty at point x
\newcommand{\vhx}{\sh^2(\xv)}          % local estimated variance at point x
\newcommand{\mh}{\hat{\mu}}            % local estimated mean
\newcommand{\mhx}{\mh(\boldsymbol{x})} % local estimated mean at point x
\)

$$\operatorname{AEI}(\xv) = \left( y^{\ast}_{\min} - \mhx \right) \Phi \left( \frac{y^{\ast}_{\min} - \mhx)}{\shx} \right) + \shx \phi \left( \frac{y^{\ast}_{\min} - \mhx}{\shx} \right) \cdot \left(1 - \frac{\sde}{\sqrt{\sde^2 + \vhx}} \right)$$
