...
As you can see from the console output of `parallelMap` the intial design will also be evaluated in parallel.
...
Note that this time we rely on MBO to autmaically use the random forest for the surrogate as the objective function contains the discrete variable `x1`.
Also we now specify the number of initial desing points manually according to the number of levels of the discrete variable.
...
ctrl = setMBOControlTermination(control = iters = 5)
...
In some cases it is acutally possible to parallize the objective function.
This can also be done with the help of `parallelMap`.

### Custom objective function

One simple case to make use of parallelization is when we have a noisy function and we just evaluate it repeatadly to reduce the noisyness.
The following example illustrates that, but also can be usefull to show you how to parallize your objective in a more sophisticated way.
...
res.parallel
```
As you can see in the output of `parallelMap` we now parallize the level `custom.objective` and not as previously `mlrMBO.feval`.
If you forget to register your own level `parallelMap` will parellelize the first call of `parallelMap` and that would be the call we do internally to evaluate the proposed points.
