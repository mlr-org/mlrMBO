...
# Purpose
This Vignette is supposed to give you an introduction on how to use `mlrMBO` for hyperparameter 
tuning in the context of machine learning using the `mlr` package. 

# `mlr`
For the purpose of hyperparameter tuning, we will use the `mlr` package. 
`mlr` provides a framework for machine learning in R that comes with a broad range of machine learning functionalities and is easily extendable. One possible approach is to use `mlr` to train a learner and evaluate its performance for a given hyperparameter configuration in the objective function. Alternatively, we can access `mlrMBO`'s model-based optimization directly using `mlr`'s tuning functionalities. This yields the benefit of integrating hyperparameter tuning with model-based optimization into your machine learning experiments without any overhead. For further information about `mlr`, please visit http://mlr-org.github.io.

# Preparations
First, we load the required packages. Next, we configure `mlr` to suppress the learner output to improve output readability. Additionaly, we define a global variable giving the number of tuning iterations. Note that this number is set (very) low to reduce runtime.
...
As an example, we tune the `cost` and the `gamma` parameter of a rbf-SVM on the [Iris data](https://en.wikipedia.org/wiki/Iris_flower_data_set). First, we define the parameter set. Note that the transformations added in the `trafo` argument mean, that we tune the parameters on logscale.
...
Next, we define the objective function. To evaluate the performance, we define a learner and set its hyperparameters by using `makeLearner` and then train it and evaluate its performance using the `resample` function. Resampling is done using 3-fold cross-validation, passing the `ResampleDesc` object `cv3`, that comes predefined with `mlr`, as an argument to `resample`. Like [here](https://mlr-org.github.io/mlrMBO/articles/supplementary/mixed_space_optimization.html "Mixed Space Optimization"), we set `minimize = TRUE` and `has.simple.signature = FALSE`. Note that the `iris.task` is provided automatically when loading `mlr`.
...
Instead of defining an objective function where the learner's performance is evaluated, we can make
use of model-based optimization directly from `mlr`. We just create a `TuneControl` object, passing the `MBOControl` object to it. Then we call `tuneParams` to tune the hyperparameters.
...
We can easily add multi point proposals and parallelize it using the `parallelMap` package. (Note that the chosen *multicore* back-end for parallelization does not work on windows machines. Please refer to the [parallelization section](https://mlr-org.github.io/mlrMBO/articles/supplementary/parallelization.html) for details on parallelization and multi point proposals.) In each iteration, we propose as many points as CPUs used for parallelization. As infill criterion we use Expected Improvement.
...
Next, we create the control objects and a suitable learner, combining `makeFilterWrapper()` with `makeModelMultiplexer()`. (Please refer to [this page](http://mlr-org.github.io/mlr-tutorial/release/html/advanced_tune/index.html#tuning-across-whole-model-spaces-with-modelmultiplexer) for details.)
