...
This vignette gives a short example about the usage of the different infill criteria in `mlrMBO`.
The infill criteria plays an important part of the optimization process in MBO as it is responsible for the proposals of new points at each iteration and thus steers the search.
...
Most crierions take the mean and variance estimate of the surrogate model into account, so that the choice of infill criteria is connected to the choice of the surrogate regression method.
...
To specify which infill criterion should be used, the `MBOControl` object hast to be extended by calling `setMBOControlInfill()`.
...
For infill criteria that use the variance of the surrogate model you need a learner that supports the variance estimate.
...
First, we will use the *mean response* and second the *expected improvement* as infill criterion, while leaving the other settings the same.
...
Hence, in case of minimization, it takes the minima of the surrogate model and in case of maximization it takes the maxima.
...
Note that normally, we would set `predict.type = "se"` but as the **mean** as infill criterion doesn't take the variance estimate into account we don't have to.
...
However, it's also worth to consider the estimated mean because areas with low mean are more interesting.
