...
We construct an examplarly objective function using `smoof::makeSingleObjetiveFunction()`. 
...
The objective function will be 3-dimensional with the inputs `j`, `k` and `method`.
`j` is in the interval [0,1] and `k` as an integer in {1, 2}. 
...
# call the function for some values
...
For *maximization* the *upper* Confidence Bound is given by $UCB(x) = \hat{\mu}(x) - \lambda \cdot \hat{s}(x)$.
We set $\lambda = 5$.
...
The inital design is set to size of 12:

```{r init_design}
design2 = generateDesign(n = 4, par.set = getParamSet(objfun2))
```
...
If we had 2 discrete variables one with 5 and one with 3 discrete values the inital design should be at least of size 15.
Usually `generateDesign()` takes care that the points are spread uniformly.

### Optimization

Finally, we start the maximation with `mbo()` with surpressed learner output from *mlr* and we print the result object to obtain the input that lead to the best objective:
```{r mbo_run, eval=TRUE}
...
Visualization is only possible for 2-dimensional objective functions.
Therefore we define a simplified objective function:

```{r objective_function_2d}
fun = function(x) {
  j = x$j
  method = x$method
...
objfun3 = makeSingleObjectiveFunction(
  ...
  fn = fun,
  par.set = makeParamSet(
    makeNumericParam("j", lower = 0,upper = pi),
    makeDiscreteParam("method", values = c("a", "b"))
  ),
  has.simple.signature = FALSE,
  minimize = FALSE
...
We also generate a simplified design:

```{r init_design_2d}
design3 = generateGridDesign(getParamSet(objfun3), resolution = 4)
```

Let's start the optimization:

```{r example_run_2d}
ex.run3 = exampleRun(objfun3, design = design3, learner = surr.rf, control = control2)
```
